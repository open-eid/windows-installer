...
param(
   ...
   [string]$filename = "Open-EID-$msiversion$env:VER_SUFFIX.exe",
...
& $candle -nologo metainfo.wxs "-dMSI_VERSION=$msiversion"
& $light -nologo -out "metainfo.msi" metainfo.wixobj
& $candle -nologo uninstaller.wxs "-dMSI_VERSION=$msiversion"
...
& $candle bootstrapper.wxs -nologo -ext WixBalExtension -ext WixUtilExtension "-dMSI_VERSION=$msiversion" `
    "-dupdater=$updater" "-dqesteidutil=$qesteid" "-dqdigidoc=$qdigidoc" "-dminidriver=$minidriver" `
    "-dieplugin=$ieplugin" "-dchrome=$chrome" "-dfirefox=$firefox" "-dloader=$loader" "-dURL=$url" "-dembed=$embed"
& $light bootstrapper.wixobj -nologo -ext WixBalExtension -out "$filename"
if($sign) {
    cp "$filename" "unsigned"
    & $insignia -nologo -ib "$filename" -o engine.exe
    Sign("engine.exe")
    & $insignia -nologo -ab engine.exe "$filename" -o "$filename"
    Sign("$filename")
}
